...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Crop Info Modal (unchanged for now)
    ...
    if (cropInfoBtn && cropModal && closeModal && cropModalTitle && cropModalContent && cropTypeSelect) {
        ...
        console.error('Crop info modal elements missing');
    }

    // Real-time Validation (unchanged for now)
    const inputs = {
    ...
    // Toast Notification (unchanged for now)
        ...
        recommendationForm.addEventListener('submit', async function (e) {
            ...
            if (!outputDiv || !errorCard || !growthPredictionCard || !buttonText || !loadingSpinner || !progressBar || !progress) {
                console.error('One or more output elements not found:', {
                    outputDiv, errorCard, growthPredictionCard, buttonText, loadingSpinner, progressBar, progress
                });
                return;
            }
